...
for i_snr = 1:length(EbN0dB)
    ...
    while (err_stat(2) < nbr_erreur && err_stat(3) < nbr_bit_max)
        ...
        b_encode = encodage_LDPC(b,g);
        x      = step(mod_psk,  b_encode); % Modulation BPSK
        ...
        Lc_decod = decodage_LDPC(Lc,h);
        rec_b = double(Lc_decod(1:K) < 0); % Décision
        T_rx    = T_rx + toc(rx_tic);  % Mesure du débit de décodage
        
        err_stat   = step(stat_erreur, b, Lc_decod(4:6)); %step(stat_erreur, b, rec_b); % Comptage des erreurs binaires
